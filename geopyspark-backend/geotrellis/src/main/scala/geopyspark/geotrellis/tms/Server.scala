...
object TMSServer {
  ...
  //   val route = new ValueReaderRoute(reader, catalog, new RenderSinglebandFromCM(cm))
  //   new TMSServer(route)
  // }
  ...
  // def serveCatalogFromURI(uriString: String, cm: ColorMap): TMSServer = {
  //   import geotrellis.spark.io.s3._
  //   val attribStore = AttributeStore(java.net.URI(uriString))
  //   val reader = S3ValueReader(bucket, root)
  //   val route = new ValueReaderRoute(reader, catalog, new RenderSinglebandFromCM(cm))
  //   new TMSServer(route)
  // }

  // def serveCatalogFromURICustom(uriString: String, display: TileRender): TMSServer = {
  //   import geotrellis.spark.io.s3._
  //   val reader = S3ValueReader(bucket, root)
  //   val route = new ValueReaderRoute(reader, catalog, display)
  //   new TMSServer(route)
  ...
  //   }
  //   val route = new SpatialRddRoute(level_map, new RenderSinglebandFromCM(cm), AkkaSystem.system)
  //   new TMSServer(route)
  // }
