...
def get_rdd(pysc):
    sc = pysc._jsc.sc()
    ew = pysc._gateway.jvm.ArrayMultibandTileWrapper
...
if __name__ == "__main__":
    ...
    java_import(sc._gateway.jvm, "geopyspark.geotrellis.ArrayMultibandTileWrapper")

    (arrays, schema) = get_rdd(sc)

    new_arrays = arrays.map(lambda s: [x + 3 for x in s])
