...
class TileLayerMethodsTest(BaseTestClass):
    ...
    hadoop_rdd = get(BaseTestClass.geopysc, SPATIAL, dir_path)
    ...
    value = hadoop_rdd.to_numpy_rdd().collect()[0]

    rasterio_rdd = BaseTestClass.geopysc.pysc.parallelize([(value[0], tile_dict)])
    raster_rdd = RasterRDD.from_numpy_rdd(BaseTestClass.geopysc, SPATIAL, rasterio_rdd)
    ...
    metadata = hadoop_rdd.collect_metadata(extent=extent, layout=layout)

    def test_cut_tiles_hadoop(self):
        result = self.hadoop_rdd.cut_tiles(self.metadata)

        (key_bounds, tile) = result.to_numpy_rdd().collect()[0]
    ...
    def test_cut_tiles_rasterio(self):
        result = self.raster_rdd.cut_tiles(self.metadata)
