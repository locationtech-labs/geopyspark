...
class ProjectedExtentSchemaTest(unittest.TestCase):
    ...
    def test_encoded_pextents(self):
        ...
        encoded = rdd.map(lambda s: ge.projected_extent_encoder(s))

        actual_encoded = encoded.collect()

        expected_encoded = [
                {'epsg': 2004, 'extent': ge.extent_encoder(self.extents[0])},
                {'epsg': 2004, 'extent': ge.extent_encoder(self.extents[1])},
                {'epsg': 2004, 'extent': ge.extent_encoder(self.extents[2])}
                ]
